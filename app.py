        ...
        with get_db() as (conn, cur):
            ...
            if access_row and access_row["sections"]:
                ...
                if user["role"] == "chairman":
                    sections = ["attendance","leave","salary","chat","leads","sales","chatlogs","fulldata","resume"]
                else:
...
AVAILABLE_SECTIONS = ["attendance","leave","salary","sales","leads","chat","fulldata","chatlogs","resume"]
        ...
        try:
            ...
            if response.status_code == 429:
                if attempt < MAX_RETRIES - 1:
                    ...
                    # Parse Gemini's retry-after header if present
                    retry_after = response.headers.get("Retry-After", 60)
                    return jsonify({
                        "message": f"Gemini is busy. Please wait {retry_after}s and try again.",
            ...
            return jsonify({"message": "Request timed out — try again"}), 504
        except Exception as e:
            return jsonify({"message": str(e)}), 500

    return jsonify({"message": "Still rate limited after retries. Wait 1–2 minutes."}), 429
...
def groq_proxy():
    ...
    # Cap at 8192 — Groq's per-request limit
        ...
        try:
            ...
            if "rate_limit" in err_str or "429" in err_str:
                if attempt < MAX_RETRIES - 1:
                    ...
                    time.sleep(wait)
                    continue
                return jsonify({"message": "Rate limit hit. Wait a moment and try again.", "retryAfter": 30}), 429
